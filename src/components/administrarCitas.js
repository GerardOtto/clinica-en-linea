...
import Modal from './modal'; // Asegúrate de que la ruta al archivo sea correcta
...
const VerCitas = ({ inSesion }) => {
  ...
  const handleEditClick = (cita) => {
    setEditCita(cita);
    setFormValues(cita);
    // Desplazar la página después de que el formulario haya sido renderizado
    setTimeout(() => {
      if (formRef.current) {
  ...
  return (
    <div className="ver-citas">
      ...
      {editCita && (
        <div ref={formRef} className="editar-cita-form-container">
          <form className="editar-cita-form" onSubmit={handleFormSubmit}>
            ...
            <label>
              Estado:
              <input type="text" name="estado" value={formValues.estado} onChange={handleInputChange} />
            </label>
            <button type="submit">Guardar cambios</button>
            <button type="button" onClick={() => setEditCita(null)}>Cancelar</button>
          </form>
        </div>
      )}
      {modalImage && (
